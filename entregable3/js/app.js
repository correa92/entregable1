document.body.onload = function () {
  ...
  for (const elem of peliculasEnCartelera) {
    let pelisEnCarteleraDOM = document.getElementById("tablaMain");
    let contenedorTR1 = document.createElement("tr");
    contenedorTR1.innerHTML = `
    ...
    pelisEnCarteleraDOM.appendChild(contenedorTR1);
  }
  ...
  while (!salir) {
    ...
    switch (eleccion) {
      case 1:
        ...
        if (carrito.length > 0) {
          let resultado = carrito.find(
            (elem) => elem.nombrePelicula === pedido.nombrePelicula
          );
          let resultado2 = carrito.find(
            (elem) => elem.formato === pedido.formato
          );
          ...
          if (!(resultado && resultado2)) {
            ...
            alert(
              "La película con este formato ya se encuentra en el carrito, seleccione otra."
            );
          }
        } else {
          carrito.push(pedido);
      ...
      if (carrito.length < 1) {
        ...
        if(mensajeCarrito.classList.contains('off')){
          mensajeCarrito.classList.remove("off");
          salir = true;
        }
          break;
